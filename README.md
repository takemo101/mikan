...
mikan stores every Issue as Markdown under `.mikan/`. The CLI, TUI, local Browser board, MCP server, and watcher all operate on those same files.
...
`mikan browser` opens a local Web board over the same Markdown files.
...
Full board behavior, raw HTML handling, guardrails, and deferred surfaces are documented in the manual: <https://takemo101.github.io/mikan/browser>
...
`mikan skills add` is **separate** from MCP registration. It installs a compact mikan `SKILL.md` using each agent's native Agent Skills convention so the agent follows a board-first operating loop, uses MCP before CLI, falls back to the `mikan` CLI only when MCP is unavailable, and handles single-project versus workspace Repository rules. Installing skills never changes MCP config, and registering MCP never installs skills.

```sh
mikan skills add --agent <agent> [--no-global]
```

Supported skill agents: `pi`, `antigravity`, `jcode`, `claude-code`, `opencode`, `codex`, `copilot-vscode`, `copilot-cli`.
...
- no shared, hosted, or always-on Browser server; `mikan browser` is a local foreground process bound to `127.0.0.1`;

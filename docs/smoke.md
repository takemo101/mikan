...
- `mikan tui` starts the OpenTUI board. Manual TUI smoke path:
  1. use arrow keys to move between Cards and Columns;
  2. press Enter/Return to open the selected Issue detail view;
  3. press Esc to return to the board;
  4. press `m`, choose a target Status, and press Enter to move the selected Issue;
  5. press `a`, type a short Note, and press Enter to append it to `## Notes`;
  6. press `q` to quit.

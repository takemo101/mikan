...
- **Repository filter**: in workspace mode, filter Cards by primary `repository` only in the current release; `affects` does not widen results unless an explicit include-affected scope is active. The active filter and the selected Issue are reflected in URL query parameters such as `?repository=backend&issue=MIK-123`, so a reload restores them.
...
## Planned action design

The next Browser action slices keep the Board focused on display, filtering, selection, and drag-and-drop Status moves. Label, Archive, and GitHub Mirror actions are designed for the Issue detail modal.

### Repository filter scope

Workspace Repository filtering keeps primary-only as the default and adds a segmented scope control:

```txt
Repository: [ backend v ]   Scope: [ Primary | +Affected ]
```

`Primary` shows Issues whose primary `repository` matches the selected Repository. `+Affected` also includes Issues whose `affects` contains the selected Repository. The URL stores the expanded scope as `includeAffected=1`, for example `?repository=backend&includeAffected=1&issue=MIK-123`.

`affects` remains display/filter context only. It never chooses the GitHub Mirror target.

### Detail actions

The detail modal gets a top action bar below the header:
...
- **Edit labels** opens a small popover inside the detail modal. Config-defined Labels are checkboxes in config order. Unknown existing Labels are shown as read-only preserved Labels. Saving updates frontmatter only; it does not add Status Log or Note entries.
- **Archive** opens a confirmation modal, then moves the Issue to Status `archived` and writes `Archived via mikan browser`. It does not delete the Markdown file, and it does not add unarchive or show-archived Browser views.
- **Create/Update GitHub Mirror** opens a confirmation modal showing the target Repository, then runs a synchronous create/update request. Workspace Mirrors use the Issue's primary `repository` and `repositories[].github.repo`; Labels and `affects` never choose the target.
...
Even after the planned action slices, the Browser still leaves the following out:
